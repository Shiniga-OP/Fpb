...
static Registrador regs[] = {
    ...
    // preservados:
    {"x19", 1, 0, 0, 1, 0, 0, 0, 0},{"x20", 1, 0, 0, 1, 0, 0, 0, 0},
    {"x21", 1, 0, 0, 1, 0, 0, 0, 0},{"x22", 1, 0, 0, 1, 0, 0, 0, 0},
    {"x23", 1, 0, 0, 1, 0, 0, 0, 0},{"x24", 1, 0, 0, 1, 0, 0, 0, 0},
    {"x25", 1, 0, 0, 1, 0, 0, 0, 0},{"x26", 1, 0, 0, 1, 0, 0, 0, 0},
    {"x27", 1, 0, 0, 1, 0, 0, 0, 0},{"x28", 1, 0, 0, 1, 0, 0, 0, 0}
};
// buscar
...
const char* alocar_reg(FILE* s, TipoToken tipo) {
    reg_cnt++;
    // tenta achar registrador livre do tipo certo
    for(int i = 0; i < 32; i++) {
        if(!regs[i].livre) continue;
        
        if(tipo == T_pFLU && regs[i].eh_flu) {
            regs[i].livre = 0;
            regs[i].sujo = 1;
            regs[i].ultimo_uso = reg_cnt;
            return regs[i].nome;
        }
        if(tipo == T_pDOBRO && regs[i].eh_dobro) {
            regs[i].livre = 0;
            regs[i].sujo = 1;
            regs[i].ultimo_uso = reg_cnt;
            return regs[i].nome;
        }
        if((tipo == T_pINT || tipo == T_pCAR) && !regs[i].eh_flu && !regs[i].eh_dobro) {
            regs[i].livre = 0;
            regs[i].sujo = 1;
            regs[i].ultimo_uso = reg_cnt;
            return regs[i].nome;
    ...
    // derrame: acha o registrador menos usado recentemente
    int menos_usado = -1;
    int min_uso = INT_MAX;
    
    for(int i = 0; i < 32; i++) {
        if(regs[i].preservado) continue; // evita derrame de preservados primeiro
        if(regs[i].ultimo_uso < min_uso) {
            min_uso = regs[i].ultimo_uso;
            menos_usado = i;
        }
    }
    if(menos_usado == -1) {
    ...
    derrame(s, regs[menos_usado].nome);
    regs[menos_usado].livre = 1;
    return alocar_reg(s, tipo);
}
...
void derrame(FILE* s, const char* reg_nome) {
    // acha o registrador na lista
    for(int i = 0; i < 32; i++) {
        if(strcmp(regs[i].nome, reg_nome) == 0) {
            // se tá sujo salva na pilha
            if(regs[i].sujo) {
                fprintf(s, "  str %s, [sp, -16]!  // [DERRAME] de %s\n", 
                        reg_nome, reg_nome);
                regs[i].sujo = 0;
            }
            regs[i].livre = 1;
...
const char* achar_pra_derrame() {
    ...
    const char* reg_escolhido = NULL;
    
    for(int i = 0; i < 32; i++) {
        // derrama registradores não preservados primeiro
        if(!regs[i].preservado && regs[i].ultimo_uso < menor_uso) {
            menor_uso = regs[i].ultimo_uso;
            reg_escolhido = regs[i].nome;
        }
    }
    // se não achou não preservado, derrama preservado
    if(reg_escolhido == NULL) {
        for(int i = 0; i < 32; i++) {
...
void proximoToken() {
    ...
    while(1) {
        ...
        if(c == '/' && L.fonte[L.pos + 1] == '*') {
            ...
            for(;;) {
                c = L.fonte[L.pos];
                if(!c) fatal("[proximoToken] comentario nao fechado");
                if(c == '\n') {
                    L.linha_atual++;
                    L.coluna_atual = 1;
                    L.pos++;
    ...
    if(c == '#') {
        ...
        fatal("[proximoToken] diretiva desconhecida");
    }
    ...
    if(isdigit((unsigned char)c) || c == '.') {
        ...
        while((c = L.fonte[L.pos]) && (isdigit((unsigned char)c) || c == '.')) {
            if(c == '.') {
                if(ponto) fatal("[proximoToken] numero invalido");
                ponto = 1;
            }
            if(i < MAX_TOK - 1) L.tk.lex[i++] = c;
            L.pos++; L.coluna_atual++;
    ...
    if(c == '"') {
        ...
        c = L.fonte[L.pos];
        while(c && c != '"') {
            if(c == '\n') fatal("[proximoToken] tex mal formado");
            if(c == '\\') {
                char n = L.fonte[L.pos + 1];
                if(!n) fatal("[proximoToken] tex mal formado");
        ...
        else fatal("[proximoToken] tex nao fechado");
        L.tk.tipo = T_TEX;
        return;
    }
    if(c == '\'') {
        L.pos++; L.coluna_atual++;
        if(!L.fonte[L.pos]) fatal("[proximoToken] caractere mal formado");
        if(L.fonte[L.pos] == '\\') {
            char n = L.fonte[L.pos + 1];
            if(!n || L.fonte[L.pos + 2] != '\'') fatal("[proximoToken] caractere mal formado");
            if(MAX_TOK > 3) sprintf(L.tk.lex, "\\%c", n);
            L.pos += 3;
            L.coluna_atual += 3;
        } else {
            char v = L.fonte[L.pos];
            if(L.fonte[L.pos + 1] != '\'') fatal("[proximoToken] caractere mal formado");
    ...
    switch(c) {
        ...
        break;
        default: fatal("[proximoToken] simbolo invalido"); break;
    }
    L.tk.lex[0] = c;
    L.tk.lex[1] = 0;
    L.pos++;
...
TipoToken tratar_id(FILE* s, int escopo) {
    ...
    if(!var) {
        ...
        } else fatal("[tratar_id] variável ou função não declarada");
    }
    proximoToken();
...
TipoToken tratar_chamada_funcao(FILE* s, int escopo, const char* nome, Funcao* fn) {
    if(fn == NULL) fatal("[tratar_chamada_funcao] INTERNO CRITICO, FUNÇÃO INEXISTENTE!");
    int param_conta = 0;
    ...
    if(L.tk.tipo != T_PAREN_DIR) {
        do {
            ...
            param_conta++;
            if(param_conta >= MAX_PARAMS) fatal("[tratar_chamada_funcao] excesso de parâmetros na chamada de função, max: 8");
...
void coletar_args(FILE* s, Funcao* f) {
    int int_reg_idc = 0;
    int fp_reg_idc = 0;
    int pilha_pos = 0; // pra parâmetros que vão para a pilha
...
// [VERIFICAÇÃO]:
void verificar_parenteses_extras() {
    if(L.tk.tipo == T_PAREN_DIR) fatal("[verificar_parenteses_extras] Parêntese extra encontrado");
}
...
void verificar_retorno(FILE* s, int escopo) {
    ...
    if(funcs[fn_cnt-1].retorno == T_pLONGO) {
        ...
        char msg[100];
        sprintf(msg, "[verificar_retorno] tipo de retorno incompatível");
        fatal(msg);
    }
...
void verificar_stmt(FILE* s, int* pos, int escopo) {
    ...
    if(L.tk.tipo == T_ID) {
        ...
        if(L.tk.tipo == T_IGUAL) {
            ...
            } else fatal("[verificar_stmt] acesso a array sem atribuição não é uma declaração válida");
            excessao(T_PONTO_VIRGULA);
            return;
