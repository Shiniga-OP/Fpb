...
void verificar_stmt(FILE* s, int* pos, int escopo) {
    ...
    if(debug_o) printf("[verificar_stmt]: token tipo: %s\n", token_str(L.tk.tipo));
    ...
    if(!eh_prototipo) {
        ...
        for(int i = 0; i < funcs[fn_cnt - 1].var_conta; i++) {
            ...
            if(var->tipo_base == T_ESPACO_ID) {
                if(var->eh_ponteiro) {
                    // ponteiro de espaço (Teste* p): salva apenas o endereço (8 bytes)
                    fp_str(s, var->reg, -param_pos);
                    fprintf(s, "  // param %s (ponteiro de espaço)\n", var->nome);
                    var->pos = -param_pos;
                    param_pos += 8;
                    continue;
                }
                /* espaço por valor (Teste p): recebemos ponteiro em var->reg (ex: x0).
...
TipoToken tratar_id(FILE* s, int escopo) {
    char id[32];
    if(debug_o) printf("[tratar_id] recebido: %s\n", id);
    strcpy(id, L.tk.lex);
    ...
    if(L.tk.tipo == T_PONTO) {
        ...
        if(L.tk.tipo == T_COL_ESQ && campo->eh_array) {
            ...
            // salva ponteiro base: x1 sera destruído pelo ldr w1 no loop de indices
            fprintf(s, "  mov x2, x1\n");
            ...
            if(dim_atual > 0) {
                ...
                // x1 = endereço base(x2) + offset(x0)
                fprintf(s, "  add x1, x2, x0\n");
            } else {
                fprintf(s, "  mov x1, x2\n");
            }
            // carrega o valor
...
void processar_args(FILE* s, Funcao* f) {
    ...
    while(L.tk.tipo != T_PAREN_DIR) {
         ...
         */
        if(eh_espaco_param) {
            // preserva eh_ponteiro definido por declaracao_var:
            // Teste p  → eh_ponteiro=0 (cópia por valor, prólogo copia o struct)
            // Teste* p → eh_ponteiro=1 (ponteiro, prólogo salva só o endereço)
            var->eh_parametro = 1;
        }
        if(L.tk.tipo == T_VIRGULA) proximoToken();
        else break;
