...
* Está é uma implementação com um limite de 8 parametros.

* Acima disso, é utilizado um sistema quebrado de pilha, que não funciona. Apenas tem o valor 0.
*/
...
#define MAX_PARAMS 9 // maximo de parametros
...
static char* arquivoAtual;// Expandir para todos os registradores disponíveis
static const char *int_param_regs[] = {"x0", "x1", "x2", "x3", "x4", "x5", "x6", "x7"};
static const char *fp_param_regs[] = {"s0", "s1", "s2", "s3", "s4", "s5", "s6", "s7", 
...
void coletar_args(FILE* s, Funcao* f) {
    int int_reg_idc = 0;
    int fp_reg_idc = 0;
    int pilha_pos = 0; // para parâmetros que vão para a pilha
